...
import { getAssignments, getGrades, submitAssignment } from "@/api/assignments";
            ...
            assignments?.map((a) => (
              <div key={a.id} className="card hover:shadow-md transition-shadow">
                <div className="flex items-start gap-4">
                  <div className={clsx("mt-1 p-2 rounded-lg", a.is_overdue ? "bg-red-50 text-red-500" : "bg-blue-50 text-blue-500")}>
                    {a.is_overdue ? <AlertCircle size={18} /> : <Clock size={18} />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-medium">{a.title}</h3>
                      <span className={clsx("text-xs px-2 py-0.5 rounded-full flex-shrink-0 capitalize", TYPE_BADGE[a.assignment_type] ?? "bg-gray-100 text-gray-600")}>
                        {a.assignment_type}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500 mt-0.5">{a.subject_name} · by {a.assigned_by_name}</p>
                    {a.description && (
                      <p className="text-sm text-gray-600 mt-1 line-clamp-2">{a.description}</p>
                    )}
                    <div className="flex items-center gap-4 mt-2">
                      <span className={clsx("text-xs font-medium", a.is_overdue ? "text-red-500" : "text-gray-500")}>
            ...
            <tbody>
              {loadingGrades ? (
                ...
                <tr key={g.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-2 px-3 text-gray-600">{g.value}/{g.max_value}</td>
